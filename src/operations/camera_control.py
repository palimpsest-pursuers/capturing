...
class CameraController(QThread):
    ...
    """
    def get_snapshot(self, hCamera):

        assert 0 != hCamera
        
        # Determine the size of buffer we'll need to hold an image from the camera
        width, height, bytesPerPixel = self.determine_raw_image_size(hCamera)
        ...
        if 0 != len(npImage):
            ...
            ret = self.get_np_image(hCamera, npImage)
    ...
    """
    def get_np_image(self, hCamera, npImage):
...
#class PreviewCamera(QThread):
    ...
    def run(self):
        ...
        if not PxLApi.apiSuccess(ret[0]):
            ...
            # return 1
        hCamera = ret[1]
        snapshot = self.get_snapshot(hCamera)
